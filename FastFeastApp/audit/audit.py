...
class Audit:
    ...
    def reset_file(self):
        ...
        self.file_null_count: int = 0

    def reset_stage(self):
        self.current_stage: str | None = None
        self.current_stage_in_records: int = 0
        self.current_stage_null_records: int = 0
    ...
    def end_batch(self):
        ...
        if self._batch_start_time:
            elapsed_ms = int((datetime.now(timezone.utc) - self._batch_start_time).total_seconds() * 1000)
        ...
        batch_msg = (
            ...
            f"| null={self.batch_null_count} | null_rate={round((self.batch_null_count / total) * 100, 2)}% "
        ...
        if self.mode == "micro_batch":
            ...
            batch_msg += f"| passed_referential_integrity={self.batch_passed_count - self.batch_orphan_count} | passed_referential_integrity_rate={round(((self.batch_passed_count - self.batch_orphan_count) / max(self.batch_passed_count, 1)) * 100, 2)}% "
    ...
    def _accumulate_to_batch(self, schema_failed: bool = False):
        ...
        self.batch_null_count += self.file_null_count
    ...
    def start_timer(self):
        ...
        self.logger.info(f"PIPELINE_START | run_id={self.run_id} | mode={self.mode} | batch_date={self.batch_date} | file={self.file_name}")
    ...
    def track_metrics(self, stats: dict[str, Any]):
        ...
        if "null_count" in stats: self.current_stage_null_records += stats["null_count"]
        ...
        self.file_null_count += self.current_stage_null_records
    ...
    def log_pipeline_end(self, schema_failed: bool = False):
        ...
        if self._file_start_time:
            elapsed_ms = int((datetime.now(timezone.utc) - self._file_start_time).total_seconds() * 1000)
            
        log_msg = (
            f"PIPELINE_END | run_id={self.run_id} | file={self.file_name} "
            f"| total={self.file_total_records} | passed={self.file_passed_count} "
            f"| duplicate={self.file_duplicate_count} | duplicate_rate={round((self.file_duplicate_count / max(self.file_total_records, 1)) * 100, 2)}% "
            f"| null={self.file_null_count} | null_rate={round((self.file_null_count / max(self.file_total_records, 1)) * 100, 2)}% "
        )
        
        if self.mode == "micro_batch":
            log_msg += (
                f"| orphan={self.file_orphan_count} | orphan_rate={round((self.file_orphan_count / max(self.file_total_records, 1)) * 100, 2)}% "
                f"| passed_referential_integrity={self.file_passed_count - self.file_orphan_count} | passed_referential_integrity_rate={round(((self.file_passed_count - self.file_orphan_count) / max(self.file_passed_count, 1)) * 100, 2)}% "
                f"| bad_rows={self.file_quarantined_count - self.file_duplicate_count - self.file_null_count - self.file_orphan_count} "
                f"| bad_rows_rate={round(((self.file_quarantined_count - self.file_duplicate_count - self.file_null_count - self.file_orphan_count) / max(self.file_total_records, 1)) * 100, 2)}% "
            )
        else:
            log_msg += (
                f"| bad_rows={self.file_quarantined_count - self.file_duplicate_count - self.file_null_count} "
                f"| bad_rows_rate={round(((self.file_quarantined_count - self.file_duplicate_count - self.file_null_count) / max(self.file_total_records, 1)) * 100, 2)}% "
            )
            
        log_msg += f"| total_quarantined={self.file_quarantined_count} | quarantine_rate={round((self.file_quarantined_count / max(self.file_total_records, 1)) * 100, 2)}% | schema_failed={schema_failed} | latency={elapsed_ms}ms | alert_sent={self.is_current_stage_alerted}"
    ...
    def log_issue(self, issue: str, value: Any, action: str):
        self.logger.warning(f"ISSUE | stage={self.current_stage} | file={self.file_name} | issue={issue} | value={value} | action={action}")

    def log_file_result(self, records_in: int, records_ok: int, records_bad: int, latency_ms: int, success: bool):
        status = "OK" if success else "FAILED"
        log_fn = self.logger.info if success else self.logger.error
        msg = (
            f"FILE_{status} | stage={self.current_stage} | file={self.file_name} "
            f"| in={records_in} ok={records_ok} bad={records_bad} "
        )
        if self.current_stage_duplicate_records > 0:
            msg += f"| duplicate={self.current_stage_duplicate_records} | duplicate_rate={round((self.current_stage_duplicate_records / max(records_in, 1)) * 100, 2)}% "
        if self.current_stage_null_records > 0:
            msg += f"| null={self.current_stage_null_records} | null_rate={round((self.current_stage_null_records / max(records_in, 1)) * 100, 2)}% "
