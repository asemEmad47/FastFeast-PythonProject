...
Logging utility for the rejected rows cleanup DAG.
...
def log_results(**context) -> None:
    ...
    cleanup_result = context["ti"].xcom_pull(task_ids="cleanup_rejected")

    log_path = os.path.join(LOG_DIR, f"rejected_cleanup_{run_id}.txt")

    with open(log_path, "w") as f:
        f.write("=" * 60 + "\n")
        f.write("  FastFeast — Rejected Rows Cleanup DAG Run Log\n")
        f.write("=" * 60 + "\n")
